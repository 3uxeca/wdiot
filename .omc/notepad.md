...
### 2026-05-29 15:55
Action execution follow-up: fixed empty LLM action payloads so buttons stay executable. `actions/executor.ts` now hydrates empty `open_tabs` payloads from captured browser/search URLs and empty `resume_work` payloads from latest captured IDE workspace/files before validator cross-checking. `resume_work` now uses Electron `shell.openPath` instead of URL-only `openExternal`. Prompt now asks LLM to include digest URLs/paths in payloads. Added `actions/executor.test.ts`. Verification: focused executor/validator/prompt tests PASS 29; `npm run typecheck --workspace @wdiot/daemon` PASS; `npx vitest run --exclude packages/daemon/src/main/ingest/ingest.test.ts` PASS 127; `npm run build` PASS.
